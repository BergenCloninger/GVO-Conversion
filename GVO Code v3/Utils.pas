...
begin
	...
	if (Date.yy < 1582)
	or ((Date.yy = 1582) and ((Date.mm < 10) or ((Date.mm = 10) and (Date.dd <= 4)))) then
		B := 0
	else
		B := 2 - A + A div 4;
...
Begin
	...
	If (InstantG < 0) Then InstantG := InstantG + 24; {InstantG = current Time of Day at Greenwich}
...
Begin
	...
	Coord(pndomem^).RA := SetStime;
	Coord(pndomem^).Dec := c_lat;
	Coord(pndomem^).RAGoTO := 0.0;
	Coord(pndomem^).DecGoTO := 0.0;
	Coord(pndomem^).RASync := SetStime;
	Coord(pndomem^).DecSync := c_lat;
	HalfSecondCounter := 0;
	CommandBuffer := '';
	XState := Tracking;
	YState := Tracking;
