...
implementation
Uses
 main,slew,CalcCoord,HandPadle,Sync, OMS68SERMC  ;
procedure TimerUpdate();
Var
...
//***********************if moving check for end of move***********************
 ...
 Begin
    ...
    Begin
      ...
      CmdStr :='AX JF'+CmdStr2 +';';     //Back to Trqacking
...
//***************************Sync Scope****************************
 If ((Coord(pndomem^).RASync<>0.0)AND(Coord(pndomem^).decSync <> 0.0)) Then SyncScope;  // sync requested
